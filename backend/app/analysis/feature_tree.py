...
class FeatureTree:
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            "project_id": self.project_id,
            "frontend": {"total": sum(frontend_counts.values()), "by_type": frontend_counts},
            "backend": {"total": sum(backend_counts.values()), "by_type": backend_counts},
        }
...
class FeatureTreeBuilder:
    ...
    def build(
        ...
        return FeatureTree(project_id=self.project_id, frontend=frontend, backend=backend)

    def _build_frontend_tree(
        self, routes: List, page_functions: Dict, api_calls: Dict
    ) -> FeatureNode:
        root = FeatureNode(
        ...
        for file_path, functions in page_functions.items():
            ...
            for func in functions:
                func_node = FeatureNode(
                    ...
                    metadata={"handler": func.handler, "function_type": func.type},
                )
    ...
    def _build_backend_tree(self, apis: List, system_features: List, models: List) -> FeatureNode:
        root = FeatureNode(
            id=self._generate_id(),
            name="后端功能",
            type=FeatureType.COMPONENT,
        ...
        for feature in system_features:
            feature_node = FeatureNode(
                ...
                metadata={
                    "feature_type": (
                        feature.type.value if hasattr(feature.type, "value") else str(feature.type)
                    ),
    ...
    def _create_route_node(self, route) -> FeatureNode:
        route_node = FeatureNode(
            ...
            metadata={"path": route.path, "component": route.component},
        )
    ...
    def _extract_page_name(self, file_path: str) -> str:
        ...
        return (
            filename.replace(".tsx", "").replace(".ts", "").replace(".jsx", "").replace(".js", "")
        )
